...
import { addManagerLog, getMachines, createMachine, updateMachine, deleteMachine, startMachineSession, stopMachineSession, getOperators } from '@/lib/admin-firestore';
...
export default function MachinesPage() {
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        ...
        if (session) {
          await addManagerLog({ operatorId: session.operatorId, operatorName: session.operatorName, action: 'update', module: 'machines', targetId: editing.id, targetName: form.name, details: `Updated machine ${form.name}` });
        }
        toast.success('Machine updated');
      } else {
        const count = machines.length + 1;
        const name = form.name || `Machine ${count}`;
        const id = await createMachine({ name, label: form.label, location: form.location, isActive: false });
        const session = getManagerSession();
        if (session) {
          await addManagerLog({ operatorId: session.operatorId, operatorName: session.operatorName, action: 'add', module: 'machines', targetId: id, targetName: name, details: `Added machine ${name}` });
        }
  ...
  async function handleDelete(m: POSMachine) {
    ...
    try {
      ...
      const session = getManagerSession();
      if (session) {
        await addManagerLog({ operatorId: session.operatorId, operatorName: session.operatorName, action: 'delete', module: 'machines', targetId: m.id, targetName: m.name, details: `Deleted machine ${m.name}` });
      }
  ...
  async function handleStart() {
    ...
    try {
      ...
      const assignedMachineIds = op.assignedMachineIds?.length ? op.assignedMachineIds : op.assignedMachineId ? [op.assignedMachineId] : [];
      if (assignedMachineIds.length > 0 && !assignedMachineIds.includes(showStartModal.id)) {
        toast.error(`${op.name} is not assigned to ${showStartModal.name}`);
        return;
      }
  ...
  const availableOperators = showStartModal
    ? operators.filter(op => {
      const assignedMachineIds = op.assignedMachineIds?.length ? op.assignedMachineIds : op.assignedMachineId ? [op.assignedMachineId] : [];
      return assignedMachineIds.length === 0 || assignedMachineIds.includes(showStartModal.id);
    })
    : operators;
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <p className="text-xs text-gray-500">{machines.length} machines &nbsp;·&nbsp; <span className="text-green-600 font-medium">{activeMachines} active</span>{isManagerMode ? ' · add/edit/delete counters' : ''}</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {machines.map(m => (
              <MachineCard
                ...
                showSessionControls={!isManagerMode}
              />
            ))}
          </div>
        )}
...
function MachineCard({ machine, onStart, onStop, onEdit, onDelete, showSessionControls }: {
  machine: POSMachine;
